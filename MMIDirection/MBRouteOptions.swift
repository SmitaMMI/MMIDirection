...
//  Global  Initialization of MMI
    var stepsArr = [RouteStep]()
    var legsArr = [RouteLeg]()
    var routeArr = [Route]()
    var last_meters: Int = 0
    ...
    internal func response(from json: JSONDictionary) -> ([Waypoint]?, [Route]?) {
        ...
        return (waypoints, self.routeArr)
    }
    
    
    // MARK:- Function for get mapBox reference from mmi 
    
    
    func getMMIData(json1:JSON)
    {
        var model_data = mmiBaseClass.init(object: "")
        model_data  = mmiBaseClass.init(json: json1)
        let temp :[mmiAdvices] = model_data.results!.alternatives![0].advices!
        
        for s_advices in temp
        {
            self.setMapBoxData(withAdvices: s_advices as mmiAdvices)
        }
        
        let leg : RouteLeg = RouteLeg.init()
        leg.steps = self.stepsArr
        
        self.legsArr.append(leg)
        
        
        let route = Route.init()
        route.legs = self.legsArr
        
        
        let tempAlternatives =  model_data.results!.alternatives![0]
        route.expectedTravelTime = TimeInterval(Float (tempAlternatives.duration!))
        route.distance = CLLocationDistance(Float (tempAlternatives.length!))
        
        
        
        route.coordinates = decodePolyline(tempAlternatives.pts!, precision: 1e6)!
        self.routeArr.append(route)

    }
    
    func setMapBoxData(withAdvices: mmiAdvices)  {
        
        let selected_pts : mmiPt = withAdvices.pt!
        let maneuverLocation = CLLocationCoordinate2D(latitude: Double (selected_pts.lat!), longitude:Double (selected_pts.lng!))
        
        let step : RouteStep = RouteStep.init()
        step.maneuverLocation = maneuverLocation
        step.instructions = withAdvices.text!

        
       
        step.distance =  Double(withAdvices.meters! - last_meters)
        step.expectedTravelTime = Double (withAdvices.seconds! -  last_seconds)

        last_meters = withAdvices.meters!
        last_seconds = withAdvices.seconds!
