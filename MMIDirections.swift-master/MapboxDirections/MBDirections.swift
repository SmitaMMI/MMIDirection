...
open class Directions: NSObject {
    ...
    open func calculate(_ options: RouteOptions, completionHandler: @escaping CompletionHandler) -> URLSessionDataTask {
        var url = self.url(forCalculating: options)
        print(url)
        
        
//        *********************
        
        let mapboxurlStr = "http://apis.mapmyindia.com/advancedmaps/v1/qeexvg99ll4j2kgzhwq54hubrbwyqkrg/route?start=28.111,77.111&destination=28.22,77.22&alternatives=true&with_advices=1"

        
        
        url = NSURL(string: mapboxurlStr)! as URL
